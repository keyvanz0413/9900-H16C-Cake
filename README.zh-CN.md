...
底层采用 **分层编排架构**。意图层会先读取当前消息、最近上下文、更早上下文，以及长期本地记忆，然后判断是直接回复，还是继续进入执行流程。如果需要执行，系统会先由 **串行规划器** 生成一个或多个步骤。技能步骤先经过 **Skill 参数解析层**，再作为 YAML 声明、Python 执行的工作流运行；开放性更强的步骤则交给主智能体处理。最后由定稿器把所有步骤结果整合成一条用户可见回复，并由专用写入智能体维护用户画像、习惯和写作风格文件。
...
```mermaid
flowchart LR
    U["用户<br/>浏览器 :3300"] --> O["oo-chat<br/>Next.js UI"]

    subgraph EA["email-agent"]
        I["意图层<br/>直接回复判断入口"]
        P["规划器<br/>串行多步骤计划"]
        R["Skill 参数解析层"]
        S["Python 技能层<br/>固定工作流执行"]
        A["主智能体<br/>ReAct + 工具"]
        T["工具层<br/>邮箱 / 日历 / 联系人 / 附件 / Shell / Todo"]
        F["定稿器<br/>统一生成最终回复"]
        W["记忆留存判断<br/>+ 写入器"]
        LM[("本地记忆<br/>USER_PROFILE.md<br/>USER_HABITS.md<br/>WRITING_STYLE.md")]
...
- **意图层** 是整个系统的入口，它会读取当前轮输入、近期与较早对话上下文，以及 `USER_PROFILE.md`、`USER_HABITS.md`、`WRITING_STYLE.md`。
- 如果当前请求不能直接回复，**规划器** 会生成一个**串行多步骤**执行计划，而不是只产出单步动作。
- **Skill 步骤** 会先经过 **Skill 参数解析层**，再进入 Python 工作流；**Agent 步骤** 则直接交给 **主智能体**。
- 这两条执行路径共享同一套底层工具，包括 Gmail / Outlook、日历、Shell、Todo、附件抽取和退订工具。
- **定稿器** 会把所有完成步骤的结果整合成一条最终回复，之后再由记忆留存判断与写入器决定哪些信息应该回写到长期本地记忆中。
